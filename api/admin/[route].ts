...
async function handleGetUploadUrl(req: any, res: any) {
  ...
  const isTemplate = typeof fileName === "string" && /^templates\/[\w\-]{1,200}\.(pdf|docx)$/.test(fileName) && !fileName.includes("..");

  if (!isPreview && !isPdf && !isAudio && !isInfo && !isTemplate)
    return res.status(400).json({ error: "Invalid file path. Use 'notes/', 'samples/', 'previews/', 'audio/', 'infographics/', or 'templates/' prefixes." });
