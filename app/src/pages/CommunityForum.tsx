...
export default function CommunityForum() {
  ...
  return (
    <div className="min-h-screen bg-parchment">
      <Navbar />
      <SEO 
        title="EduLaw Community Forum — Legal Discussions & Q&A"
        description="Join India's most active legal student community. Ask questions about BNS, BNSS, BSA, Judiciary prep, and NLU life."
        canonical="/community"
      />
      ...
      <section className="relative pt-32 pb-20 overflow-hidden">
        ...
        <div className="absolute top-40 right-10 w-32 h-32 opacity-20 animate-float">
          <Users className="w-full h-full text-burgundy" />
        </div>
        <div className="absolute bottom-20 left-10 w-28 h-28 opacity-20 animate-float-delayed">
          <MessageSquare className="w-full h-full text-gold" />
        </div>
        ...
        <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          >
            <div className="inline-flex items-center gap-2 px-4 py-2 bg-burgundy/10 rounded-full mb-6">
              ...
              <span className="font-ui text-sm text-burgundy">Join the Discussion</span>
            </div>
            
            <h1 className="font-display text-5xl md:text-6xl text-ink mb-6">
              Community <span className="text-gold">Forum</span>
            </h1>
            ...
            <div className="flex flex-wrap justify-center gap-8 mt-12">
              <div className="text-center">
                <div className="font-display text-3xl text-burgundy">{forumTopics.length}+</div>
                <div className="font-ui text-sm text-mutedgray">Topics</div>
              </div>
              <div className="text-center">
                <div className="font-display text-3xl text-burgundy">{totalReplies}</div>
                <div className="font-ui text-sm text-mutedgray">Replies</div>
              </div>
              <div className="text-center">
                <div className="font-display text-3xl text-burgundy">{solvedTopics}</div>
      ...
      <section className="py-6 border-y border-parchment-dark bg-white/50">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex flex-wrap items-center justify-between gap-4">
            <button className="flex items-center gap-2 px-6 py-3 bg-burgundy text-parchment rounded-xl font-ui font-semibold hover:shadow-lg hover:shadow-burgundy/30 transition-all">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          {filteredTopics.length === 0 ? (
            <div className="text-center py-16">
              ...
              <button className="px-6 py-3 bg-burgundy text-parchment rounded-xl font-ui font-semibold">
                Ask a Question
              </button>
            </div>
          ) : (
            <div className="space-y-4">
              {filteredTopics.map((topic, index) => (
                <motion.div
                  ...
                  className="group bg-white rounded-2xl border border-parchment-dark p-6 hover:shadow-lg hover:shadow-burgundy/10 transition-all cursor-pointer"
                >
                  <div className="flex items-start gap-4">
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2 mb-2">
                        <h3 className="font-display text-lg text-ink group-hover:text-burgundy transition-colors line-clamp-1">
                          {topic.title}
                        </h3>
                        {topic.isSolved && (
                      ...
                      <div className="flex flex-wrap items-center gap-3">
                        ...
                        {topic.tags.slice(0, 3).map(tag => (
                          <span key={tag} className="px-2 py-1 bg-gold/10 text-gold rounded-full font-ui text-xs">
