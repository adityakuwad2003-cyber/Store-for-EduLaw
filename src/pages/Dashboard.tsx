...
function TemplateCard({ purchase }: { purchase: Purchase }) {
  ...
  const triggerDownload = async (fileKey: string, type: 'pdf' | 'docx') => {
    ...
    try {
      ...
      const res = await fetch('/api/get-download-link', {
        ...
        body: JSON.stringify({ fileName: fileKey, productId: purchase.productId }),
      });
  ...
  const hasPdf = !!purchase.pdfKey;
  const hasDocx = !!purchase.docxKey;
      ...
      <div className="p-5">
        ...
        {!hasPdf && !hasDocx ? (
          ...
          <div className="space-y-2">
            {hasDocx && (
              <button
                onClick={() => triggerDownload(purchase.docxKey!, 'docx')}
            ...
            {hasPdf && (
              <button
                onClick={() => triggerDownload(purchase.pdfKey!, 'pdf')}
                disabled={downloading !== null}
